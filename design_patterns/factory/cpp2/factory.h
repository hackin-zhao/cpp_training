...
template <typename T, typename U, typename... Args> class factory_registrator;
...
template <typename T, typename... Args> class factory
{
  public:
    ...
    template <typename U> using registrator = factory_registrator<T, U, Args...>;
    template <typename U> using reg = factory_registrator<T, U, Args...>;
    ...
    template <typename TYPE_KEY> std::unique_ptr<T> create(TYPE_KEY key_impl_str, Args &&...data) const
    {
...
{
  public:
    template <class = typename std::enable_if<(has_instance<typename T::factory>::value)>::type>
    explicit factory_registrator() : _f(T::factory::instance())
    {
        _register(make_int_sequence<sizeof...(Args)>{});
    }

    explicit factory_registrator(factory<T, Args...> &f) : _f(f)
    {
        _register(make_int_sequence<sizeof...(Args)>{});
    }
...
template <typename Result, typename... Args> struct code
{
...
template <typename Result, typename... Args> using repository = typename dp14::code<Result, Args...>::factory;
