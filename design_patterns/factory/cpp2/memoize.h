...
template <typename T, typename U, typename... Args> class memoize_registrator;

template <typename T, typename... Args> class memoize;
...
template <typename TYPE_KEY> size_t get_hash(TYPE_KEY key_impl_str)
{
    return std::hash<TYPE_KEY>()(key_impl_str);
}

template <> size_t get_hash<const char *>(const char *key_impl_str)
{
...
template <typename T, typename... Args> class memoize
{
  public:
    ...
    template <typename U> using reg = memoize_registrator<T, U, Args...>;

    template <typename U> static typename std::enable_if<(has_key<U>::value), key_impl>::type get_key()
    {
    ...
    template <typename U, typename F> void register_type(F &&value)
    {
    ...
    template <typename U> void unregister_type()
    {
        key_impl keyimpl = get_key<U>();
        auto it = _map_registrators.find(keyimpl);
        if (it != _map_registrators.end())
    ...
    template <typename TYPE_KEY> inline bool exists(TYPE_KEY keyimpl_str, Args &&...data) const
    {
        auto keyimpl = detail::memoize::get_hash(keyimpl_str);
        return exists(keyimpl, std::forward<Args>(data)...);
    }

    template <typename U> inline bool exists(Args &&...data) const
    {
        return exists(get_key<U>(), std::forward<Args>(data)...);
    }

    template <typename TYPE_KEY> std::shared_ptr<T> get(TYPE_KEY keyimpl_str, Args &&...data) const
    {
        auto keyimpl = detail::memoize::get_hash(keyimpl_str);
    ...
    template <typename TYPE_KEY> auto execute(TYPE_KEY keyimpl_str, Args &&...data) const
    {
        auto keyimpl = detail::memoize::get_hash(keyimpl_str);
        key_cache key = get_base_hash(keyimpl, std::forward<Args>(data)...);
    ...
    template <typename TYPE_KEY> void clear(TYPE_KEY keyimpl_str, Args &&...data) const
    {
        auto keyimpl = detail::memoize::get_hash(keyimpl_str);
        key_cache key = get_base_hash(keyimpl, std::forward<Args>(data)...);
    ...
    template <typename U> inline std::shared_ptr<U> get(Args &&...data) const
    {
...
{
  public:
    template <class = typename std::enable_if<(has_instance<typename T::memoize>::value)>::type>
    explicit memoize_registrator() : _m(T::memoize::instance())
    {
        _register(make_int_sequence<sizeof...(Args)>{});
    }

    explicit memoize_registrator(memoize<T, Args...> &m) : _m(m)
    {
        _register(make_int_sequence<sizeof...(Args)>{});
    }
  ...
  protected:
    template <int... Is> void _register(int_sequence<Is...>)
    {
...
template <typename Result, typename... Args> struct code_once
{
...
template <typename Result, typename... Args> using repository_once = typename dp14::code_once<Result, Args...>::memoize;
